...
// All buttons fire through the existing onAction(id) pipe (mirrors ActionBubble) — no backend change:
//   view_cv · view_cover_letter · download · review_analysis · review_changes · review_audit · start_over
//
// Props:
...
export function CompletionBubble({ meta = {}, onAction }) {
  ...
  return (
    <div className="animate-fade-in-up w-full max-w-[85%] mx-auto bg-surface-2 border border-line rounded-2xl px-6 py-7 text-center shadow-[var(--shadow-panel)]">
      ...
      {/* conversion CTA */}
      <button
        onClick={fire('download')}
        className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-accent hover:brightness-110 text-accent-fg text-sm font-semibold shadow-[var(--shadow-panel)] transition-all active:scale-[0.99]"
      >
        <DownloadGlyph /> Download both (.pdf)
      </button>

      {/* secondary links */}
      <div className="flex items-center justify-center gap-2 mt-[18px] text-xs text-fg-muted">
        <button onClick={fire('review_analysis')} className="hover:text-fg-secondary transition-colors">See the analysis</button>
        <span className="text-fg-faint">·</span>
        <button onClick={fire('review_changes')} className="hover:text-fg-secondary transition-colors">What changed</button>
        <span className="text-fg-faint">·</span>
